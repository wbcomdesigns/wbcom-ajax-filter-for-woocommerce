	...
	<div class="filter-content">
		<ul class="filter-items filter-radio  level-0">
			...
			<li class="filter-item radio level-0">
				<label>
					<input type="radio" name="filter[<?php echo esc_html( $preset_id ); ?>][<?php echo esc_attr( $tm->id ); ?>]" class="wb-ajax-filter-selectible" value="<?php echo esc_attr( $term_data->slug ); ?>" data-filter="<?php echo esc_attr( $filter_taxonomy ); ?>" <?php echo ( isset( $params[ $filter_taxonomy ] ) && in_array( $term_data->slug, (array) $params[ $filter_taxonomy ], true ) ) ? 'checked' : ''; ?>>
