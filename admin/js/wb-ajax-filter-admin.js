	...
	jQuery( document ).ready(
		function ($) {
				...
				jQuery( ".wb-ajax-filters-single-container" ).sortable(
					{
						...
						update: function (event, ui) {
							...
							jQuery( '.wb-ajax-filters-single-container' ).find( '.wb-ajax-filter-toggle-row' ).each(
								function (index, elm) {
			...
			function searchOutputTab() {
				...
				jQuery( 'input[name="wb_ajax_filter_admin_customization_options[ajax_loader_style]"]' ).on(
					'change',
					function () {
			...
			function initializeSelect2(){
				...
				jQuery( '#wb_ajax_filter_select2_terms' ).on(
					'select2:select',
					function ( e ) {
			...
			jQuery( '.wb-ajax-filter-modal-content' ).on(
				'change',
			...
			jQuery( 'input[name="wb_ajax_filter_admin_general_options[show_reset]"]' ).on(
				'change',
				function () {
			...
			jQuery( 'input[name="filters[multiple]"]' ).on(
				'change',
				function () {
					if ( jQuery( this ).is( ':checked' ) ) {
			...
			jQuery( '.wb-ajax-filter-modal-content' ).on(
				'change',
				'.wb-filter-type-price-range-unlimited',
				function () {
					if ( jQuery( this ).is( ':checked' ) ) {
			...
			jQuery( 'input[name="wb_ajax_filter_preset_title"]' ).on(
				'keyup',
				function () {
			...
			jQuery( '.wb-ajax-filter-modal-content' ).on(
				'click',
				'.wb-ajax-filter-range-remove',
				function ( e ) {
					e.preventDefault();
					jQuery( this ).parent().remove();
					jQuery( '.wb-ajax-filter-range-box' ).each(
						function () {
			...
			jQuery( '.wb-ajax-filter-modal-content' ).on(
				'click',
				'.wb-ajax-filter-add-price-range',
				function ( e ) {
					e.preventDefault();
					let nonce = wbcom_plugin_installer_params.wbcom_ajax_nonce;
			...
			jQuery( '.wb-ajax-filter-modal-content' ).on(
				'change',
				'input[name="filters[show_toggle]"]',
				function () {
					if ( jQuery( this ).is( ':checked' ) ) {
						jQuery( '.wb-show-style-toggle' ).show();
					} else {
			...
			jQuery( '.wb-ajax-filter-modal-content' ).on(
				'change',
				'select[name="filters[filter_design]"]',
				function () {
			...
			jQuery( '.preset-active-status' ).on(
				'change',
				function () {
			...
			// Enable/Disable filter preset
			jQuery( 'input[name="filter_enabled"]' ).on(
				'change',
				function () {
					let preset    = jQuery( this ).data( 'preset' );
					let filter_id = jQuery( this ).data( 'filter_id' );
			...
			jQuery( '.wb-ajax-filter-save-title-button' ).on(
				'click',
				function ( e ) {
					e.preventDefault();
			...
			jQuery( '.wb-ajax-filter-add-button' ).on(
				'click',
				function (e) {
					e.preventDefault();
			...
			jQuery( '.wb-ajax-filter-modal-body' ).on(
				'click',
				'.wb-ajax-filter-close-modal',
				function () {
					jQuery( '.wb-ajax-filter-modal-container' ).css(
						{
							'opacity': 0,
			...
			jQuery( '.wb-ajax-filter-modal-content' ).on(
				'click',
				'#wb-ajax-filer-create-filter-save',
				function ( e ) {
					e.preventDefault();
			...
			jQuery( '.wb-ajax-filter-modal-content' ).on(
				'change',
				'.wb-filter-type-price-range-min',
				function (e) {
			...
			jQuery( '.wb-ajax-filter-modal-content' ).on(
				'change',
				'.wb-filter-type-price-range-max',
				function (e) {
			...
			jQuery( 'input[name="wb_ajax_filter_search_settings[enable_search]"]' ).on(
				'click',
				function (e) {
