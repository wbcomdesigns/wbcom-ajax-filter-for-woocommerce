 ...
 * @return void
 */
function edd_wbcom_ajax_filter_plugin_updater() {
	// retrieve our license key from the DB.
	$license_key = trim( get_option( 'edd_wbcom_ajax_filter_license_key' ) );

	// setup the updater.
	$edd_updater = new EDD_WB_Ajax_Filter_Plugin_Updater(
		EDD_WB_AJAX_FILTER_STORE_URL,
		WB_AJAX_FILTER_PLUGIN_FILE,
		array(
			'version'   => WB_AJAX_FILTER_VERSION,   // current version number.
			'license'   => $license_key,        // license key (used get_option above to retrieve from DB).
			'item_name' => EDD_WB_AJAX_FILTER_ITEM_NAME,  // name of this plugin.
			'author'    => 'wbcomdesigns',  // author of this plugin.
			'url'       => home_url(),
		)
	);
}
add_action( 'admin_init', 'edd_wbcom_ajax_filter_plugin_updater', 0 );
 ...
 * @return void
 */
function edd_wbcom_ajax_filter_register_option() {
	// creates our settings in the options table.
	register_setting( 'edd_wbcom_ajax_filter_license', 'edd_wbcom_ajax_filter_license_key', 'edd_ajax_filter_sanitize_license' );
}
add_action( 'admin_init', 'edd_wbcom_ajax_filter_register_option' );
 ...
 * @return void
 */
function edd_wbcom_ajax_filter_activate_license() {
	// listen for our activate button to be clicked.
	if ( isset( $_POST['edd_ajax_filter_license_activate'] ) ) {
		// run a quick security check.
		if ( ! check_admin_referer( 'edd_wbcom_ajax_filter_nonce', 'edd_wbcom_ajax_filter_nonce' ) ) {
			return; // get out if we didn't click the Activate button.
		}

		// retrieve the license from the database.
		$license = isset( $_POST['edd_wbcom_ajax_filter_license_key'] ) ? sanitize_text_field( wp_unslash( $_POST['edd_wbcom_ajax_filter_license_key'] ) ) : '';

		// data to send in our API request.
		$api_params = array(
			'edd_action' => 'activate_license',
			'license'    => $license,
			'item_name'  => urlencode( EDD_WB_AJAX_FILTER_ITEM_NAME ), // the name of our product in EDD.
			'url'        => home_url(),
		);
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( false === $license_data->success ) {
				switch ( $license_data->error ) {
					case 'expired':
						$message = sprintf(
							...
							__( 'Your license key expired on %s.', 'wb-ajax-filter' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = __( 'Your license key has been disabled.', 'wb-ajax-filter' );
						break;

					case 'missing':
						$message = __( 'Invalid license.', 'wb-ajax-filter' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = __( 'Your license is not active for this URL.', 'wb-ajax-filter' );
						break;

					case 'item_name_mismatch':
						/* translators: %1$s: Item Name*/
						$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'wb-ajax-filter' ), EDD_WB_AJAX_FILTER_ITEM_NAME );
						break;

					case 'no_activations_left':
						$message = __( 'Your license key has reached its activation limit.', 'wb-ajax-filter' );
						break;
				...
				set_transient( 'edd_wbcom_ajax_filter_license_key_data', $license_data, 12 * HOUR_IN_SECONDS );
			}
		}
...
add_action( 'admin_init', 'edd_wbcom_ajax_filter_activate_license' );
 ...
 * @return void
 */
function edd_wbcom_ajax_filter_deactivate_license() {
	// listen for our activate button to be clicked.
	if ( isset( $_POST['edd_ajax_filter_license_deactivate'] ) ) {
...
add_action( 'admin_init', 'edd_wbcom_ajax_filter_deactivate_license' );
	...
	if ( $pagenow === 'plugins.php' || $pagenow === 'index.php' || ( isset( $_GET['page'] ) && $_GET['page'] === 'wbcom-license-page' ) ) { //phpcs:ignore
		...
		if ( empty( $license_data ) && '' !== $license ) {
			...
			$api_params = array(
				...
				'license'    => $license,
				'item_name'  => urlencode( EDD_WB_AJAX_FILTER_ITEM_NAME ),
				'url'        => home_url(),
			);
 ...
 */
function edd_wbcom_ajax_filter_admin_notices() {
	$license_activation = filter_input( INPUT_GET, 'wb_ajax_filter_activation' ) ? filter_input( INPUT_GET, 'wb_ajax_filter_activation' ) : '';
 ...
 * @return void
 */
function edd_wbcom_ajax_filter_render_license_section() {
		...
		<table class="form-table wb-license-form-table">
			<tr>
				...
				<td class="wb-plugin-license-key">
					<input id="edd_wbcom_ajax_filter_license_key" name="edd_wbcom_ajax_filter_license_key" type="text" class="regular-text" value="<?php echo esc_attr( $license ); ?>" />
