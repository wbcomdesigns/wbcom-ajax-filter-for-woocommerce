...
if ( ! class_exists( 'YIT_Plugin_Panel' ) ) {
	...
	class YIT_Plugin_Panel {
		...
		public function __construct( $args = array() ) {
			if ( ! empty( $args ) ) {
				...
				if ( isset( $args['parent_page'] ) && 'yit_plugin_panel' === $args['parent_page'] ) {
					$args['parent_page'] = 'wb-ajaxfilter';
				}
		...
		protected function maybe_init_vars() {
			if ( ! isset( $this->main_array_options ) && ! isset( $this->tabs_hierarchy ) ) {
				...
				foreach ( $this->settings['admin-tabs'] as $item => $v ) {
					...
					$path = apply_filters( 'wb-ajaxfilter_item_options_path', $path, $options_path, $item, $this );
					if ( file_exists( $path ) ) {
						$_tab                     = include $path;
						...
						foreach ( $sub_tabs as $sub_item => $sub_options ) {
							...
							$sub_tab_path = apply_filters( 'wb-ajaxfilter_sub_tab_item_options_path', $sub_tab_path, $sub_tabs, $sub_item, $this );
		...
		public function add_menu_page() {
			...
			if ( ! isset( $admin_page_hooks['wb-ajaxfilter'] ) ) {
		...
		public function remove_duplicate_submenu_page() {
			remove_submenu_page( 'wb-ajaxfilter', 'wb-ajaxfilter' );
		}
		...
		public function add_premium_version_upgrade_to_menu() {
			...
			if ( apply_filters( 'yit_show_upgrade_to_premium_version', isset( $submenu['wb-ajaxfilter'] ) ) ) {
				...
				$submenu['wb-ajaxfilter']['how_to'] = $how_to_menu; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
			}
		}
		...
		public static function sort_plugins() {
			global $submenu;
			if ( ! empty( $submenu['wb-ajaxfilter'] ) ) {
				$sorted_plugins = $submenu['wb-ajaxfilter'];
				...
				$submenu['wb-ajaxfilter'] = $sorted_plugins; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
			}
		}
		...
		public static function add_menu_class_in_yith_plugin( $menu ) {
			...
			if ( ! empty( $submenu['wb-ajaxfilter'] ) ) {
				$item_count = count( $submenu['wb-ajaxfilter'] );
